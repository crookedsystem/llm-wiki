...
- 書き込まれた synthesized/meta note の provenance trailer
- `GET /metrics` REST endpoint で vault と graph counters を統合して提供
- `kb_search_notes` MCP tool による LLM Wiki Markdown 検索
- `kb_wiki_context` による schema-first wiki map、link issue candidates、update suggestions の提供
- `kb_validate_vault` による vault schema validation
- `kb_reconcile_taxonomy` による deterministic tag taxonomy reconciliation
- raw note metadata と body-only sha256 check を含む `kb_write_note` の schema-enforced write
...
- `kb_write_note` を通じて完全な Markdown note を書き込む。サーバーは書き込み前に schema violation を拒否する。
- Wiki 作業は `kb_wiki_context` から始め、drafting 前に allowed tags、page types、index、recent log、schema health を確認する。
- `kb_validate_vault` と `kb_reconcile_taxonomy` を deterministic schema hygiene と tag taxonomy repair に使う。
...
現在サーバーが公開している MCP tool は `kb_write_note`、`kb_search_notes`、`kb_wiki_context`、`kb_validate_vault`、`kb_reconcile_taxonomy` です。Vault/graph counters は REST `GET /metrics` endpoint で公開します。
